  ...
  <body>
    <header class="container-fluid bg-dark text-light p-3">
      <div class="row">
        ...
        <div class="col d-flex justify-content-end"> @auth <a href="/dashboard" class="text-light text-decoration-none mx-2">Profile</a>
          <a href="/cart" class="text-light text-decoration-none mx-2">Cart</a>
          <a href="/payment" class="text-light text-decoration-none mx-2">Payment</a> @endauth @guest <a href="/login" class="text-light text-decoration-none mx-2">Login</a>
          <a href="/register" class="text-light text-decoration-none mx-2">Register</a> @endguest
        </div>
      </div>
    </header>
    <div class="container mt-5">
      <div class="row">
        <div class="col-md-6">
          <h1>{{ $shop->name }}</h1>
          <p>Alamat: {{ $shop->alamat }}</p> @php $paragraphs = explode("\n", $shop->deskripsi); // Split text by newlines @endphp @foreach ($paragraphs as $paragraph) <p>{{ $paragraph }}</p> @endforeach <br>
        </div>
      </div>
